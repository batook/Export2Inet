...
public class SVFile extends XmlFile {

	public SVFile(Connection oraSession, FileProperties fileProps) {
		super(oraSession, fileProps);
		companyName = "Техносила";
		companyFullName = "Интернет-магазин СВ";
		url = "http://www.sv.ru";
	...
	protected void createXml(String fileName) throws SQLException, XMLStreamException, IOException {
		...
		ResultSet grp_cursor = grp_stmnt.executeQuery("select * from goods.v_price_export2site");
		...
		while (cursor.next()) {
			...
			writer.writeCharacters(goodsPic + cursor.getString("itemid") + ".jpg");
			writer.writeEndElement();
